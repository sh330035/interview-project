      ...
      <div class="fixed z-10 inset-0 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen">
          ...
          <div class="bg-white rounded-lg w-[450px] h-[700px] z-30 relative">
            ...
            <button @click="closeModal" class="top-0 right-0 mt-[28px] mr-[28px] text-[#595959] absolute">
            ...
            <form class="px-[65px] mt-[15px]" v-if="activeTab === 'login'">
                <div class="flex flex-col">
                    <p class="text-base text-[#595959] text-center mb-[25px]">使用 HiSKIO ID 登入</p>
                    <div class="relative">
                        <input v-model="email" class="shadow appearance-none border-hidden rounded w-full pl-[35px] pr-4 py-2 mb-[10px] text-gray-700 text-sm bg-[#FAFAFA] leading-tight placeholder:text-[#BFBFBF]  focus:outline-none focus:shadow-outline invalid:border-pink-500 invalid:text-pink-600" id="email" type="email" placeholder="請輸入 HiSKIO ID" required>
                        <div class="absolute inset-y-0 left-0 flex items-center pl-[7px] pb-[6px]">
                            <img src="../src/images/account.svg" alt="" class="w-6 h-6">
                        </div>
                    </div>
                    <div class="relative">
                        <input v-model="password" class="shadow appearance-none border-hidden rounded w-full pl-[35px] pr-4 py-2 text-gray-700 text-sm bg-[#FAFAFA] mb-3 leading-tight placeholder:text-[#BFBFBF]   focus:outline-none focus:shadow-outline" id="password" type="password" placeholder="請輸入登入密碼" required>
                        <div class="absolute inset-y-0 left-0 flex items-center pl-[7px] pb-[6px]">
                            <img src="../src/images/lock.svg" alt="" class="w-6 h-6">
                        </div>
                    </div>
                    <div class="flex my-[15px]">
                        <input type="checkbox" class="w-5 h-5 rounded appearance-none checked:bg-[#178FAC]" />
                        <p class="text-[#8C8C8C] text-sm ml-[5px]">登入註冊即代表您同意<a href="/" class="underline">使用者及隱私權政策</a></p>
                    </div>
                    <button class="bg-[#178FAC] hover:bg-[#10778F] text-white text-base py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="button" @click="login">
                    登入
                    </button>
